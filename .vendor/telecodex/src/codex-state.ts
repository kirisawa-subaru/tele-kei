...
export function listThreads(limit = 20): CodexThreadRecord[] {
  const result = queryThreads(limit);
  return result.available ? result.value : [];
}

export function queryThreads(limit = 20): CodexStateQuery<CodexThreadRecord[]> {
  return withAvailableDatabase((db) => {
    const query = db.prepare(`
      SELECT id, name, title, cwd, model, created_at, updated_at, first_user_message
      FROM threads
      WHERE (archived = 0 OR archived IS NULL)
        AND source IN ('vscode', 'cli')
        AND (thread_source IS NULL OR thread_source IN ('user', 'automation'))
...
export function getThread(id: string): CodexThreadRecord | null {
  const result = queryThread(id);
  return result.available ? result.value : null;
}

export function queryThread(id: string): CodexStateQuery<CodexThreadRecord | null> {
  return withAvailableDatabase((db) => {
    const query = db.prepare(`
      SELECT id, name, title, cwd, model, created_at, updated_at, first_user_message
      FROM threads
...
function withDatabase<T>(fn: (db: DatabaseInstance) => T): T | null {
  const result = withAvailableDatabase(fn);
  return result.available ? result.value : null;
}

function withAvailableDatabase<T>(fn: (db: DatabaseInstance) => T): CodexStateQuery<T> {
  if (!BetterSqlite3) {
  ...
  try {
    ...
    if (!hasLoggedDatabaseOpenFailure) {
      ...
      console.error(`Failed to open Codex state database at ${databasePath}:`, error);
    }
    return { available: false };
  }

  try {
    return { available: true, value: fn(db) };
  } catch {
    return { available: false };
  } finally {
    try {
