import { findLatexSpans } from "./latex.js";
...
function extractLatex(text: string, latex: string[]): string {
  const spans = findLatexSpans(text);
  ...
  for (const span of spans) {
    ...
    const index = latex.push(span.raw) - 1;
    output += `${LATEX_PREFIX}${index}${LATEX_SUFFIX}`;
    cursor = span.end;
  }
