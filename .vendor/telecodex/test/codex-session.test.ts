...
describe("CodexSessionService", () => {
  ...
  it("create accepts overrides for workspace, model, and resumeThreadId", async () => {
    const service = await CodexSessionService.create(createConfig(), {
  ...
  it("can defer thread creation while preserving the fixed SDK policy", async () => {
    const service = await CodexSessionService.create(createConfig(), {
    ...
    expect(mockState.createdThreads[0].options).toEqual({
      model: "o3",
      sandboxMode: "workspace-write",
      workingDirectory: "/workspace/base",
      approvalPolicy: "never",
  ...
  it("reports per-turn token usage without exposing session totals", async () => {
    const service = await CodexSessionService.create(createConfig());
    const thread = mockState.createdThreads[0];
    ...
    expect(service.getInfo()).not.toHaveProperty("sessionTokens");
  });
