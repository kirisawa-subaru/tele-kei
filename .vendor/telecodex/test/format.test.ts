...
describe("formatTelegramHTML", () => {
  ...
  it("preserves inline and display TeX without interpreting markdown markers", () => {
    const input = "Inline $x_**i**^2$ and \\(a_*_b\\).\n\n$$c_**d**$$";
    expect(formatTelegramHTML(input)).toBe(input);
  });
