...
describe("context-key", () => {
  ...
  it("derives the key from a grammy context", () => {
    const ctx = {
      chat: { id: 67890, type: "supergroup", is_forum: true },
  ...
  it("extracts context key from callback query message_thread_id", () => {
    const ctx = {
      chat: { id: 67890, type: "supergroup", is_forum: true },
      message: undefined,
      callbackQuery: { message: { message_thread_id: 99, is_topic_message: true } },
    } as unknown as Context;

    expect(contextKeyFromCtx(ctx)).toBe("67890:99");
  });

  it("does not split a private chat merely because Telegram includes a thread id", () => {
    const ctx = {
      chat: { id: 12345, type: "private" },
      message: { message_thread_id: 77 },
    } as unknown as Context;
    expect(contextKeyFromCtx(ctx)).toBe("12345");
  });
